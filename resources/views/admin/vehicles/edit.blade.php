                ...
                <div class="card-body">
                    <form action="{{ route('admin.vehicles.update', $vehicle->id) }}" method="POST">
                        ...
                        <div class="form-input">
                            ...
                            <input type="text" class="form-input" name="licence_plate_number" id="type" pattern="[A-Z]{2}\d{4}[A-Z]{2}" value="{{ $vehicle->licence_plate_number }}" required>
                        </div>
                        <div class="form-input">
                            <label for="city">City</label>
                            <select required name="city_id" id="city" class="form-input">
                                @foreach ($cities as $city)
                                    <option @if ($city->id == $vehicle->city_id)
                                        selected
                        ...
                        <div class="form-input">
                            ...
                            <select name="vehicle_type_id" id="vehicle_type" class="form-input" required>
                                @foreach ($vehicle_types as $vehicle_type)
                                    <option @if ($vehicle_type->id == $vehicle->vehicle_type_id)
                                        selected
                                    @endif value="{{ $vehicle_type->id }}">{{ $vehicle_type->type }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-input">
